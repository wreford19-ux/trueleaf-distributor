...
export default function App() {
  ...
  const submit = async () => {
    const order = {
      ref:"TL-"+Date.now().toString().slice(-6), when:new Date().toISOString(), createdAt:serverTimestamp(),
      ...
      totalUnits, lineCount, status:"Pending",
    };
    setView("thanks");
    try { await addDoc(collection(db,"orders"), order); } catch(e){ console.error(e); }
  ...
  return (
    <div style={S.page}>
      ...
      <div style={{background:"#fbf4dd", borderBottom:"1px solid "+PAL.border, color:PAL.brown, fontSize:12, padding:"7px 16px", textAlign:"center"}}>
        <strong>Preview mode</strong> — accounts, the order email to Greg, and the confirmation email to the distributor are simulated here; they go live on deploy.
      </div>
