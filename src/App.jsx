...
import { Search, ShoppingCart, X, Plus, Minus, LogOut, Leaf, ChevronLeft, Trash2, Check, ZoomIn, Send,
  ShieldCheck, Download, Upload, Package, ClipboardList, Sprout, AlertTriangle, CheckCircle2,
  Eye, EyeOff, Users, UserPlus, Power, Ban, FileDown, Pencil } from "lucide-react";
import * as XLSX from "xlsx";
import { auth, db, createDistributorAuth } from "./firebase";
...
const byId = (id) => SEED_PRODUCTS.find(p=>p.id===id);
const lbl = { display:"flex", flexDirection:"column", gap:5, fontSize:12, fontWeight:700, color:"#5c3d1e" };
const iconBtn = (PAL) => ({ display:"inline-flex", alignItems:"center", justifyContent:"center", width:30, height:30, borderRadius:7, border:"1px solid "+PAL.border, background:PAL.card, color:PAL.green, cursor:"pointer" });
...
export default function App() {
  ...
  const [stock, setStock] = useState(() => { const s={}; SEED_PRODUCTS.forEach(p=>s[p.id]=p.stock); return s; });
  ...
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, async (fb) => {
      ...
      try {
        ...
        if (prof.exists() && prof.data().active === false) {
          setUser(null); setAuthReady(true);
          await signOut(auth);
          show("This distributor account has been deactivated. Please contact TrueLeaf to reactivate it.");
          return;
        }
        setUser(prof.exists()
  ...
  // ---- Firebase: live catalogue (after sign-in) ----
  useEffect(() => {
    if (!user && !adminAuthed) return;
    const unsub = onSnapshot(doc(db, "catalog", "products"),
      (snap) => { const d = snap.data(); if (snap.exists() && d && Array.isArray(d.items) && d.items.length) setProducts(d.items); },
      () => {});
    return unsub;
  }, [user, adminAuthed]);
  ...
  const counts = useMemo(() => { const m={All:products.length}; products.forEach(p=>m[p.cat]=(m[p.cat]||0)+1); return m; }, [products]);
  const filtered = useMemo(() => {
    ...
    return list;
  }, [cat, search, sort, products]);

  const cartLines = useMemo(() => Object.keys(cart).filter(id=>Number(cart[id])>0).map(id=>({...(prodById[+id]||{}), qty:Number(cart[id])||0})).filter(x=>x.id!==undefined), [cart, prodById]);
  const totalUnits = cartLines.reduce((s,l)=>s+(Number(l.qty)||0),0);
  const lineCount = cartLines.length;
  const setQty = (id, qty) => setCart(c=>({...c,[id]:Math.max(0, Number(qty)||0)}));
  const addOne = (id) => setCart(c=>({...c,[id]:(Number(c[id])||0)+1}));
  ...
  // ---------- ADMIN ----------
  if (adminAuthed) return <Admin {...{orders,setOrders,stock,setStock,products,setProducts,setAdminAuthed,S,PAL,show}} />;
  ...
  const submit = async () => {
    const base = {
      ...
      lines:cartLines.map(l=>({id:l.id,code:l.code,name:l.name,cat:l.cat,qty:Number(l.qty)||0})),
  ...
  return (
    <label style={{display:"block", gridColumn:span?"1 / -1":"auto"}}>
      ...
      <div style={{position:"relative"}}>
        <input type={isPw && reveal ? "text" : type} value={v} onChange={e=>on(e.target.value)} style={{...S.input, paddingRight:isPw?40:12}}/>
        {isPw && <button type="button" tabIndex={-1} onClick={()=>setReveal(r=>!r)} title={reveal?"Hide password":"Show password"} style={{position:"absolute", right:6, top:"50%", transform:"translateY(-50%)", background:"none", border:"none", cursor:"pointer", color:PAL.muted, padding:6, display:"flex"}}>{reveal?<EyeOff size={16}/>:<Eye size={16}/>}</button>}
      </div>
    </label>
  );
}

function AddDistributor({onClose,onAdd,S,PAL}) {
  const [f,setF] = useState({business:"",contact:"",phone:"",email:"",password:"",addr1:"",suburb:"",city:"",province:"",postal:""});
  const [busy,setBusy] = useState(false);
  const up=(k,v)=>setF(s=>({...s,[k]:v}));
  const submit=async()=>{ setBusy(true); await onAdd(f, ()=>onClose()); setBusy(false); };
  return (
    <div onClick={onClose} style={{position:"fixed", inset:0, background:"rgba(20,15,8,0.5)", display:"flex", alignItems:"flex-start", justifyContent:"center", padding:"40px 16px", zIndex:50, overflowY:"auto"}}>
      <div onClick={e=>e.stopPropagation()} style={{background:PAL.card, border:"1px solid "+PAL.border, borderRadius:12, padding:22, width:"100%", maxWidth:560}}>
        <div style={{display:"flex", alignItems:"center", marginBottom:14}}><h3 style={{...S.serif, color:PAL.green, margin:0, fontSize:19}}>Add a distributor</h3><div style={{flex:1}}/><button onClick={onClose} style={{background:"none", border:"none", cursor:"pointer", color:PAL.muted, display:"flex"}}><X size={20}/></button></div>
        <div style={{display:"grid", gridTemplateColumns:"1fr 1fr", gap:10}}>
          ...
          <Field label="Temp password *" v={f.password} on={v=>up("password",v)} S={S} PAL={PAL} type="password"/>
        ...
        <p style={{fontSize:11.5, color:PAL.muted, margin:"10px 0 0"}}>Give them this temp password — they can change it later via "Forgot password?" on the login screen.</p>
        <button onClick={submit} disabled={busy} style={{...S.btn, width:"100%", padding:12, marginTop:14, opacity:busy?0.7:1}}>{busy?"Adding…":"Create distributor account"}</button>
      </div>
    </div>
  );
}
...
function Auth({authMode,setAuthMode,S,PAL,show}) {
  ...
  const forgot=async()=>{ if(!String(f.email).trim()){ setErr("Enter your email above, then tap reset."); return; } try{ await sendPasswordResetEmail(auth, f.email.trim()); show("Password reset link sent — check your email."); setErr(""); }catch(e){ setErr(authErr(e)); } };
    ...
    <div style={{minHeight:"100vh", background:PAL.paper, color:PAL.ink, display:"flex", alignItems:"center", justifyContent:"center", padding:20, fontFamily:"ui-sans-serif,system-ui,sans-serif"}}>
      <div style={{width:"100%", maxWidth:authMode==="register"?560:400}}>
        ...
        <div style={{background:PAL.card, border:"1px solid "+PAL.border, borderRadius:12, padding:22}}>
          ...
          {authMode==="login" && <div style={{textAlign:"center", marginTop:10}}><button type="button" onClick={forgot} style={{background:"none", border:"none", color:PAL.muted, fontSize:12.5, cursor:"pointer", textDecoration:"underline"}}>Forgot password?</button></div>}
        </div>
...
// ===================== ADMIN =====================
function Admin({orders,setOrders,stock,setStock,products,setProducts,setAdminAuthed,S,PAL,show}) {
  ...
  useEffect(() => onSnapshot(collection(db, "distributors"),
    (snap) => setDistributors(snap.docs.map(d => ({ uid: d.id, ...d.data() }))), () => {}), []);
  useEffect(() => { (async () => {
    const cref = doc(db, "catalog", "products");
    const snap = await getDoc(cref);
    if (!snap.exists()) await setDoc(cref, { items: SEED_PRODUCTS });
  })(); }, []);
  useEffect(() => { (async () => {
    const lref = doc(db, "inventory", "levels");
    const snap = await getDoc(lref);
    const lv = snap.exists() ? {...snap.data()} : {}; let __ch = !snap.exists(); products.forEach(p=>{ if(lv[p.id]===undefined){ lv[p.id]=p.stock??0; __ch=true; } }); if(__ch) await setDoc(lref, lv);
  })(); }, [products]);

  const daysAgo = (iso) => (Date.now()-new Date(iso).getTime())/86400000;
  const inPeriod = (o) => daysAgo(o.when) <= (period==="week"?7:31);
  const orderUnits = (o) => (o.lines||[]).reduce((s,l)=>s+(Number(l.qty)||0),0);
  const periodOrders = orders.filter(inPeriod);

  const byDist = useMemo(()=>{ const m={}; periodOrders.forEach(o=>{ if(!m[o.business]) m[o.business]={business:o.business, units:0, orders:0, cats:{}}; m[o.business].units+=orderUnits(o); m[o.business].orders+=1; o.lines.forEach(l=>{m[o.business].cats[l.cat]=(m[o.business].cats[l.cat]||0)+(Number(l.qty)||0);}); }); return Object.values(m).sort((a,b)=>b.units-a.units); }, [periodOrders]);

  const totalUnits = periodOrders.reduce((s,o)=>s+orderUnits(o),0);
  ...
  const confirmOrder = async (o) => {
    try {
      await runTransaction(db, async (tx) => {
        ...
        o.lines.forEach(l => { lv[l.id] = Math.max(0, (lv[l.id] ?? 0) - (Number(l.qty)||0)); });
  ...
  const stockList = useMemo(()=>{ const q=stockSearch.trim().toLowerCase(); return products.filter(p=>q===""||p.name.toLowerCase().includes(q)||p.code.toLowerCase().includes(q)); }, [stockSearch, products]);
  const catCats = CATS.filter(c=>c!=="All");
  const prodList = useMemo(()=>{ const q=prodSearch.trim().toLowerCase(); return products.filter(p=>q===""||p.name.toLowerCase().includes(q)||p.code.toLowerCase().includes(q)||String(p.cat).toLowerCase().includes(q)); }, [products, prodSearch]);
  const saveCatalog = async (items) => { setProducts(items); try{ await setDoc(doc(db,"catalog","products"), {items}); }catch(e){ show("Could not save catalogue"); throw e; } };
  const blankProd = () => ({ id:null, code:"", name:"", cat:catCats[0], img:"", stock:0 });
  const saveProd = async () => {
    const fp = prodEdit; if(!fp) return;
    if(!fp.name.trim()||!fp.code.trim()){ show("Code and name are required"); return; }
    const img = fp.img && fp.img.trim() ? fp.img.trim() : null;
    if(fp.id===null){
      const newId = products.reduce((m,p)=>Math.max(m,p.id),-1)+1;
      const rec = { id:newId, code:fp.code.trim(), name:fp.name.trim(), cat:fp.cat, img, stock:parseInt(fp.stock)||0 };
      try{ await saveCatalog([...products, rec]); }catch(e){ return; }
      const nx = { ...stock, [newId]: parseInt(fp.stock)||0 }; setStock(nx); setDoc(doc(db,"inventory","levels"), nx).catch(()=>{});
      show(fp.name.trim()+" added");
    } else {
      const items = products.map(p=> p.id===fp.id ? { ...p, code:fp.code.trim(), name:fp.name.trim(), cat:fp.cat, img } : p);
      try{ await saveCatalog(items); }catch(e){ return; }
      show(fp.name.trim()+" updated");
    }
    setProdEdit(null);
  };
  const deleteProd = async (p) => {
    if(!window.confirm('Delete "'+p.name+'" from the catalogue? Past orders keep their record. This cannot be undone.')) return;
    try{ await saveCatalog(products.filter(x=>x.id!==p.id)); show(p.name+" deleted"); }catch(e){}
  };

  const filteredOrders = useMemo(()=>{ const q=orderSearch.trim().toLowerCase(); return orders.filter(o=>(orderFilter==="All"||o.status===orderFilter)&&(q===""||(o.business||"").toLowerCase().includes(q)||(o.ref||"").toLowerCase().includes(q))); }, [orders, orderFilter, orderSearch]);
  const distStats = useMemo(()=>{ const m={}; distributors.forEach(d=>{ m[d.uid]={...d, oCount:0, oUnits:0, last:null}; });
    orders.forEach(o=>{ let k=(o.uid&&m[o.uid])?o.uid:Object.keys(m).find(x=>m[x].email&&o.email&&m[x].email.toLowerCase()===String(o.email).toLowerCase()); if(!k) return; m[k].oCount+=1; m[k].oUnits+=orderUnits(o); if(!m[k].last||new Date(o.when)>new Date(m[k].last)) m[k].last=o.when; });
    return Object.values(m); }, [distributors, orders]);
  const distList = useMemo(()=>{ const q=distSearch.trim().toLowerCase(); return distStats.filter(d=>(showArchived?true:!d.archived)&&(q===""||(d.business||"").toLowerCase().includes(q)||(d.contact||"").toLowerCase().includes(q)||(d.email||"").toLowerCase().includes(q))).sort((a,b)=>b.oUnits-a.oUnits); }, [distStats, distSearch, showArchived]);
  const activeCount = distStats.filter(d=>!d.archived&&d.active!==false).length;

  const declineOrder = async (o) => { if(!window.confirm("Decline order "+o.ref+"? Stock will not change.")) return; try{ await updateDoc(doc(db,"orders",o.id),{status:"Declined"}); show("Order "+o.ref+" declined"); }catch(e){ show("Could not decline order"); } };
  const deleteOrder = async (o) => { if(!window.confirm("Delete order "+o.ref+" permanently? This cannot be undone.")) return; try{ await deleteDoc(doc(db,"orders",o.id)); show("Order "+o.ref+" deleted"); }catch(e){ show("Could not delete order"); } };
  const exportOrders = () => {
    const rows=[]; orders.forEach(o=>{ (o.lines||[]).forEach(l=>{ rows.push({Ref:o.ref, Date:new Date(o.when).toLocaleDateString("en-ZA"), Business:o.business, Contact:o.contact, Phone:o.phone, Email:o.email, Status:o.status, Code:l.code, Seed:l.name, Category:l.cat, Qty:Number(l.qty)||0}); }); });
    if(rows.length===0){ show("No orders to export"); return; }
    const ws=XLSX.utils.json_to_sheet(rows); const wb=XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb,ws,"Orders");
    const out=XLSX.write(wb,{type:"array",bookType:"xlsx"}); const blob=new Blob([out],{type:"application/octet-stream"});
    const url=URL.createObjectURL(blob); const a=document.createElement("a"); a.href=url; a.download="TrueLeaf_Orders_"+new Date().toISOString().slice(0,10)+".xlsx"; a.click(); URL.revokeObjectURL(url); show("Orders exported");
  };
  const toggleActive = async (d) => { const next=!(d.active===false?false:true); try{ await updateDoc(doc(db,"distributors",d.uid),{active:next}); show(d.business+(next?" activated":" deactivated")); }catch(e){ show("Could not update distributor"); } };
  const removeDistributor = async (d) => { if(!window.confirm("Remove "+d.business+"? They'll be deactivated and archived (order history is kept).")) return; try{ await updateDoc(doc(db,"distributors",d.uid),{active:false, archived:true}); show(d.business+" removed"); }catch(e){ show("Could not remove distributor"); } };
  const addDistributor = async (form, done) => {
    for(const k of ["business","contact","phone","email","password","addr1","city","province","postal"]){ if(!String(form[k]).trim()){ show("Please complete all required fields"); return; } }
    if(form.password.length<6){ show("Password must be at least 6 characters"); return; }
    try{ const uid=await createDistributorAuth(form.email.trim(), form.password);
      await setDoc(doc(db,"distributors",uid), {email:form.email.trim(), business:form.business, contact:form.contact, phone:form.phone, addr1:form.addr1, suburb:form.suburb, city:form.city, province:form.province, postal:form.postal, active:true, createdAt:serverTimestamp(), createdByAdmin:true});
      show(form.business+" added"); done();
    }catch(e){ show(/email-already-in-use/.test(String((e&&e.code)||e)) ? "That email already has an account" : "Could not add distributor"); }
  };
    ...
    <div style={{minHeight:"100vh", background:PAL.paper, color:PAL.ink, fontFamily:"ui-sans-serif,system-ui,sans-serif"}}>
      ...
      <div style={{maxWidth:1100, margin:"0 auto", padding:"16px"}}>
        <div style={{display:"flex", gap:6, marginBottom:16}}>
          {[["overview","Overview",ClipboardList],["orders","Orders",Package],["distributors","Distributors",Users],["products","Products",Leaf],["stock","Stock",Sprout]].map(([k,lab,Ic])=>(
        ...
        {tab==="overview" && (
          <div>
            ...
            <div style={{display:"flex", gap:12, marginBottom:18, flexWrap:"wrap"}}>
              ...
              <Stat label="Units ordered" value={totalUnits.toLocaleString()} PAL={PAL} S={S}/>
                  ...
                  {byDist.map((d,i)=>{
                    ...
                    return (
                      <div key={d.business} style={{display:"flex", flexDirection:"column", alignItems:"center", width:120}}>
                        <div style={{fontFamily:"ui-monospace,Menlo,monospace", fontWeight:700, fontSize:15, color:PAL.brown, marginBottom:4}}>{d.units.toLocaleString()}</div>
        ...
        {tab==="orders" && (
          <div>
            <div style={{display:"flex", alignItems:"center", gap:10, marginBottom:12, flexWrap:"wrap"}}>
              <h2 style={{...S.serif, color:PAL.green, margin:0, fontSize:22}}>Orders in & out</h2><div style={{flex:1}}/>
              <button onClick={exportOrders} style={{...S.btnGhost, display:"flex", alignItems:"center", gap:6, padding:"8px 13px"}}><FileDown size={15}/> Export</button>
            </div>
            <div style={{display:"flex", gap:6, marginBottom:10, flexWrap:"wrap", alignItems:"center"}}>
              {["All","Pending","Confirmed","Declined"].map(fs=>(<button key={fs} onClick={()=>setOrderFilter(fs)} style={{padding:"6px 13px", borderRadius:20, border:"1px solid "+(orderFilter===fs?PAL.green:PAL.border), background:orderFilter===fs?PAL.green:PAL.card, color:orderFilter===fs?"#f4ecd6":PAL.ink, fontWeight:700, fontSize:12.5, cursor:"pointer"}}>{fs}{fs==="Pending"&&pendingCount>0?" ("+pendingCount+")":""}</button>))}
              <div style={{flex:1, minWidth:160, position:"relative"}}>
                <Search size={15} color={PAL.muted} style={{position:"absolute", left:10, top:10}}/>
                <input style={{...S.input, paddingLeft:32, padding:"8px 12px 8px 32px"}} placeholder="Search by business or ref…" value={orderSearch} onChange={e=>setOrderSearch(e.target.value)}/>
              </div>
            </div>
            {filteredOrders.length===0 && <p style={{color:PAL.muted}}>No orders match.</p>}
            <div style={{display:"grid", gap:10}}>
              {filteredOrders.map(o=>{
                const open = expanded===o.ref; const pend = o.status==="Pending"; const declined = o.status==="Declined";
                return (
                  <div key={o.ref} style={{background:PAL.card, border:"1px solid "+(pend?"#e0b94e":PAL.border), borderRadius:10, overflow:"hidden"}}>
                    <div onClick={()=>setExpanded(open?null:o.ref)} style={{display:"flex", alignItems:"center", gap:12, padding:"12px 14px", cursor:"pointer"}}>
                      ...
                      <div style={{flex:1}}>
                        ...
                        <div style={{fontSize:12, color:PAL.muted}}>{o.lineCount} lines · {orderUnits(o).toLocaleString()} units</div>
                      </div>
                      <span style={{fontSize:12, fontWeight:700, padding:"4px 10px", borderRadius:20, background:declined?"#f3dcd5":(pend?"#fbeec4":"#dcebd0"), color:declined?"#9e3b2e":(pend?"#86631a":PAL.green), display:"inline-flex", alignItems:"center", gap:5}}>
                        {declined?<Ban size={13}/>:pend?<AlertTriangle size={13}/>:<CheckCircle2 size={13}/>}{o.status}
                      </span>
                      {pend && <button onClick={(e)=>{e.stopPropagation(); confirmOrder(o);}} style={{...S.btn, padding:"7px 13px", fontSize:13}}>Confirm</button>}
                      {pend && <button onClick={(e)=>{e.stopPropagation(); declineOrder(o);}} style={{...S.btnGhost, padding:"7px 13px", fontSize:13, borderColor:"#b5341f", color:"#b5341f"}}>Decline</button>}
                      <button onClick={(e)=>{e.stopPropagation(); deleteOrder(o);}} title="Delete order" style={{background:"none", border:"none", cursor:"pointer", color:PAL.muted, padding:6, display:"flex"}}><Trash2 size={16}/></button>
                    </div>
                    {open && (
        ...
        {tab==="distributors" && (
          <div>
            <div style={{display:"flex", alignItems:"center", gap:10, marginBottom:12, flexWrap:"wrap"}}>
              <h2 style={{...S.serif, color:PAL.green, margin:0, fontSize:22}}>Distributors</h2><div style={{flex:1}}/>
              <button onClick={()=>setAddOpen(true)} style={{...S.btn, display:"flex", alignItems:"center", gap:6, padding:"9px 14px"}}><UserPlus size={16}/> Add distributor</button>
            </div>
            <div style={{display:"flex", gap:12, marginBottom:14, flexWrap:"wrap"}}>
              <Stat label="Distributors" value={distStats.filter(d=>!d.archived).length} PAL={PAL} S={S}/>
              <Stat label="Active" value={activeCount} PAL={PAL} S={S}/>
              <Stat label="Total orders" value={orders.length} PAL={PAL} S={S}/>
            </div>
            <div style={{display:"flex", gap:8, marginBottom:12, alignItems:"center", flexWrap:"wrap"}}>
              <div style={{flex:1, minWidth:180, position:"relative"}}>
                <Search size={15} color={PAL.muted} style={{position:"absolute", left:10, top:10}}/>
                <input style={{...S.input, paddingLeft:32, padding:"8px 12px 8px 32px"}} placeholder="Search distributors…" value={distSearch} onChange={e=>setDistSearch(e.target.value)}/>
              </div>
              <label style={{fontSize:12.5, color:PAL.muted, display:"flex", alignItems:"center", gap:6, cursor:"pointer"}}><input type="checkbox" checked={showArchived} onChange={e=>setShowArchived(e.target.checked)}/> Show archived</label>
            </div>
            {distList.length===0 && <p style={{color:PAL.muted}}>No distributors yet. Add one, or wait for them to register.</p>}
            <div style={{display:"grid", gap:10}}>
              {distList.map(d=>{ const inactive=d.active===false; const open=distExpanded===d.uid;
                const dOrders=orders.filter(o=>(o.uid&&o.uid===d.uid)||(o.email&&d.email&&String(o.email).toLowerCase()===String(d.email).toLowerCase()));
                return (
                  <div key={d.uid} style={{background:PAL.card, border:"1px solid "+PAL.border, borderRadius:10, overflow:"hidden", opacity:inactive?0.65:1}}>
                    <div style={{display:"flex", alignItems:"center", gap:12, padding:"12px 14px", flexWrap:"wrap"}}>
                      <div style={{flex:1, minWidth:160, cursor:"pointer"}} onClick={()=>setDistExpanded(open?null:d.uid)}>
                        <div style={{...S.serif, fontWeight:700, fontSize:15}}>{d.business||"(no business name)"} {d.archived?<span style={{fontSize:11, color:PAL.muted}}>· archived</span>:null}</div>
                        <div style={{fontSize:12, color:PAL.muted}}>{d.contact} · {d.email} · {d.phone}</div>
                      </div>
                      <div style={{textAlign:"center", minWidth:54}}><div style={{fontWeight:800, fontFamily:"ui-monospace,Menlo,monospace", color:PAL.brown}}>{d.oCount}</div><div style={{fontSize:10, color:PAL.muted, textTransform:"uppercase"}}>orders</div></div>
                      <div style={{textAlign:"center", minWidth:72}}><div style={{fontWeight:800, fontFamily:"ui-monospace,Menlo,monospace", color:PAL.brown}}>{d.oUnits.toLocaleString()}</div><div style={{fontSize:10, color:PAL.muted, textTransform:"uppercase"}}>units</div></div>
                      <span style={{fontSize:11.5, fontWeight:700, padding:"4px 10px", borderRadius:20, background:inactive?"#eee3c8":"#dcebd0", color:inactive?PAL.muted:PAL.green}}>{inactive?"Inactive":"Active"}</span>
                      <button onClick={()=>toggleActive(d)} style={{...S.btnGhost, padding:"6px 11px", fontSize:12.5, display:"flex", alignItems:"center", gap:5}}><Power size={14}/>{inactive?"Activate":"Deactivate"}</button>
                      <button onClick={()=>removeDistributor(d)} title="Remove" style={{background:"none", border:"none", cursor:"pointer", color:PAL.muted, padding:6, display:"flex"}}><Trash2 size={16}/></button>
                    </div>
                    {open && (
                      <div style={{borderTop:"1px solid "+PAL.border, padding:"12px 14px", background:"#fdfaf0"}}>
                        <div style={{fontSize:12.5, color:PAL.muted, marginBottom:8}}>Deliver to: {d.addr1}{d.suburb?", "+d.suburb:""}, {d.city}, {d.province} {d.postal} · Last order: {d.last?new Date(d.last).toLocaleDateString("en-ZA",{day:"numeric",month:"short",year:"numeric"}):"—"}</div>
                        {dOrders.length===0 ? <div style={{fontSize:12.5, color:PAL.muted}}>No orders yet.</div> : (
                          <table style={{width:"100%", borderCollapse:"collapse", fontSize:12.5}}>
                            <thead><tr style={{background:"#f0e8cd", color:PAL.brown}}><th style={{textAlign:"left", padding:"5px 8px"}}>Ref</th><th style={{textAlign:"left", padding:"5px 8px"}}>Date</th><th style={{textAlign:"right", padding:"5px 8px"}}>Units</th><th style={{textAlign:"left", padding:"5px 8px"}}>Status</th></tr></thead>
                            <tbody>{dOrders.map(o=>(<tr key={o.ref} style={{borderBottom:"1px solid "+PAL.border}}><td style={{padding:"5px 8px", fontFamily:"ui-monospace,Menlo,monospace", fontWeight:700, color:PAL.brown}}>{o.ref}</td><td style={{padding:"5px 8px"}}>{new Date(o.when).toLocaleDateString("en-ZA",{day:"numeric",month:"short"})}</td><td style={{padding:"5px 8px", textAlign:"right", fontFamily:"ui-monospace,Menlo,monospace"}}>{orderUnits(o).toLocaleString()}</td><td style={{padding:"5px 8px"}}>{o.status}</td></tr>))}</tbody>
                          </table>
                        )}
                      </div>
                    )}
            ...
            {addOpen && <AddDistributor onClose={()=>setAddOpen(false)} onAdd={addDistributor} S={S} PAL={PAL}/>}
          </div>
        )}
        ...
        {tab==="products" && (
          <div>
            <div style={{display:"flex", alignItems:"center", gap:10, marginBottom:14, flexWrap:"wrap"}}>
              <h2 style={{...S.serif, color:PAL.green, margin:0, fontSize:22}}>Products</h2><div style={{flex:1}}/>
              <button onClick={()=>setProdEdit(blankProd())} style={{...S.btn, display:"flex", alignItems:"center", gap:6, padding:"9px 14px"}}><Plus size={16}/> Add product</button>
            </div>
            <div style={{display:"flex", gap:12, marginBottom:14, flexWrap:"wrap"}}>
              <Stat label="Products" value={products.length} PAL={PAL} S={S}/>
              <Stat label="Categories" value={catCats.length} PAL={PAL} S={S}/>
            </div>

            {prodEdit && (
              <div style={{background:PAL.card, border:"1px solid "+PAL.green, borderRadius:10, padding:16, marginBottom:16}}>
                <div style={{display:"flex", alignItems:"center", gap:8, marginBottom:12}}>
                  <Leaf size={17} color={PAL.green}/>
                  <strong style={{...S.serif, color:PAL.green, fontSize:16}}>{prodEdit.id===null?"New product":"Edit product"}</strong>
                  {prodEdit.id!==null && <span style={S.code}>#{prodEdit.id}</span>}
                </div>
                <div style={{display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))", gap:10}}>
                  <label style={lbl}>Code<input style={S.input} value={prodEdit.code} onChange={e=>setProdEdit({...prodEdit, code:e.target.value})} placeholder="e.g. VTMM"/></label>
                  <label style={{...lbl, gridColumn:"1 / -1"}}>Seed name<input style={S.input} value={prodEdit.name} onChange={e=>setProdEdit({...prodEdit, name:e.target.value})} placeholder="e.g. Tomato Money Maker"/></label>
                  <label style={lbl}>Category<select style={S.input} value={prodEdit.cat} onChange={e=>setProdEdit({...prodEdit, cat:e.target.value})}>{catCats.map(c=><option key={c} value={c}>{c}</option>)}</select></label>
                  {prodEdit.id===null && <label style={lbl}>Opening stock<input style={S.input} value={prodEdit.stock} onChange={e=>setProdEdit({...prodEdit, stock:e.target.value.replace(/[^0-9]/g,'')})} placeholder="0"/></label>}
                  <label style={{...lbl, gridColumn:"1 / -1"}}>Image URL<input style={S.input} value={prodEdit.img||""} onChange={e=>setProdEdit({...prodEdit, img:e.target.value})} placeholder="https://…  (leave blank for no image)"/></label>
                </div>
                <div style={{display:"flex", alignItems:"center", gap:12, marginTop:12}}>
                  {prodEdit.img && prodEdit.img.trim()
                    ? <img src={prodEdit.img} alt="" style={{width:54, height:54, objectFit:"cover", borderRadius:8, border:"1px solid "+PAL.border}} onError={e=>{e.currentTarget.style.opacity=0.25;}}/>
                    : <div style={{width:54, height:54, borderRadius:8, border:"1px dashed "+PAL.border, display:"flex", alignItems:"center", justifyContent:"center"}}><Leaf size={20} color={PAL.line}/></div>}
                  <div style={{flex:1}}/>
                  <button onClick={()=>setProdEdit(null)} style={{...S.btnGhost, padding:"9px 15px"}}>Cancel</button>
                  <button onClick={saveProd} style={{...S.btn, display:"flex", alignItems:"center", gap:6}}><Check size={16}/> {prodEdit.id===null?"Add product":"Save changes"}</button>
                </div>
                {prodEdit.id!==null && <p style={{fontSize:11.5, color:PAL.muted, marginTop:10, marginBottom:0}}>Stock levels are managed in the <strong>Stock</strong> tab.</p>}
              </div>
            )}

            <div style={{position:"relative", marginBottom:12}}>
              <Search size={16} color={PAL.muted} style={{position:"absolute", left:11, top:12}}/>
              <input style={{...S.input, paddingLeft:34}} placeholder="Find a product by name, code or category…" value={prodSearch} onChange={e=>setProdSearch(e.target.value)}/>
            </div>
            <div style={{background:PAL.card, border:"1px solid "+PAL.border, borderRadius:10, overflow:"hidden"}}>
              <div style={{display:"flex", alignItems:"center", padding:"8px 14px", background:"#f0e8cd", fontSize:11, fontWeight:700, color:PAL.brown, textTransform:"uppercase", letterSpacing:0.5}}>
                <span style={{width:44}}></span><span style={{width:66}}>Code</span><span style={{flex:1}}>Seed</span><span style={{width:130}}>Category</span><span style={{width:80, textAlign:"right"}}>Actions</span>
              </div>
              <div style={{maxHeight:520, overflowY:"auto"}}>
                {prodList.slice(0,200).map(p=>(
                  <div key={p.id} style={{display:"flex", alignItems:"center", padding:"8px 14px", borderTop:"1px solid "+PAL.border}}>
                    <div style={{width:44}}>{p.img
                      ? <img src={p.img} alt="" style={{width:34, height:34, objectFit:"cover", borderRadius:6, border:"1px solid "+PAL.border}} onError={e=>{e.currentTarget.style.visibility="hidden";}}/>
                      : <div style={{width:34, height:34, borderRadius:6, border:"1px dashed "+PAL.border, display:"flex", alignItems:"center", justifyContent:"center"}}><Leaf size={15} color={PAL.line}/></div>}</div>
                    <span style={{width:66, fontFamily:"ui-monospace,Menlo,monospace", fontSize:12, fontWeight:700, color:PAL.brown}}>{p.code}</span>
                    <span style={{flex:1, ...S.serif, fontWeight:700, fontSize:13.5}}>{p.name}</span>
                    <span style={{width:130, fontSize:12, color:PAL.muted}}>{p.cat}</span>
                    <div style={{width:80, display:"flex", justifyContent:"flex-end", gap:6}}>
                      <button onClick={()=>setProdEdit({id:p.id, code:p.code, name:p.name, cat:p.cat, img:p.img||"", stock:stock[p.id]??0})} title="Edit" style={iconBtn(PAL)}><Pencil size={15}/></button>
                      <button onClick={()=>deleteProd(p)} title="Delete" style={{...iconBtn(PAL), color:"#b5341f", borderColor:"#e0b7a3"}}><Trash2 size={15}/></button>
                    </div>
                  </div>
                ))}
                {prodList.length>200 && <div style={{padding:"10px 14px", fontSize:12, color:PAL.muted, textAlign:"center"}}>Showing first 200 — search to narrow down.</div>}
                {prodList.length===0 && <div style={{padding:"18px 14px", fontSize:13, color:PAL.muted, textAlign:"center"}}>No products match that search.</div>}
              </div>
            </div>
            <p style={{fontSize:11.5, color:PAL.muted, marginTop:10}}>The catalogue lives in Firestore (<code>catalog/products</code>) and updates live for every distributor the moment you save — no code changes needed. Deleting a product here doesn't affect past orders.</p>
          </div>
        )}
