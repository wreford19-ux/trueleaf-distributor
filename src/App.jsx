...
import { Search, ShoppingCart, X, Plus, Minus, LogOut, Leaf, ChevronLeft, Trash2, Check, ZoomIn, Send,
  ...
  Eye, EyeOff, Users, UserPlus, Power, Ban, FileDown } from "lucide-react";
...
export default function App() {
  ...
  const cartLines = useMemo(() => Object.keys(cart).filter(id=>Number(cart[id])>0).map(id=>({...byId(+id), qty:Number(cart[id])||0})).filter(x=>x.id!==undefined), [cart]);
...
function Admin({orders,setOrders,stock,setStock,setAdminAuthed,S,PAL,show}) {
  ...
  useEffect(() => { (async () => {
    ...
    const lv = snap.exists() ? {...snap.data()} : {}; let __ch = !snap.exists(); PRODUCTS.forEach(p=>{ if(lv[p.id]===undefined){ lv[p.id]=p.stock; __ch=true; } }); if(__ch) await setDoc(lref, lv);
  })(); }, []);
    ...
    <div style={{minHeight:"100vh", background:PAL.paper, color:PAL.ink, fontFamily:"ui-sans-serif,system-ui,sans-serif"}}>
      ...
      <div style={{maxWidth:1100, margin:"0 auto", padding:"16px"}}>
        <div style={{display:"flex", gap:6, marginBottom:16}}>
          {[["overview","Overview",ClipboardList],["orders","Orders",Package],["distributors","Distributors",Users],["stock","Stock",Sprout]].map(([k,lab,Ic])=>(
